...
Knowledge base for AI agents and engineers working on this repository.

This is not a summary of what the code does; the code says that. It
records the decisions behind it and the mistakes already made, because
several of them look arbitrary without the reasoning attached and
overturning one without reading this reintroduces a known bug.

**Read section 1 before writing anything.** Consult the hazard sections
when working in the area they name; each was written after a real
failure, and most of those failures produced no error at all.

## How to use this document

| If you are | Read |
|---|---|
| new to the project | 1, then 2 |
| changing numerics | 3 |
| changing attention, rotary or masking | 4 |
| changing device placement or compilation | 5 |
| changing checkpoint loading or tokenization | 6 |
| measuring performance | 7 |

A recurring theme worth internalising before anything else: **in this
codebase, most bugs do not raise.** A wrong rotary convention, a
mismatched dtype, a mis-selected hidden state, a subtly different
tokenization all produce correctly shaped output containing different
numbers. That is why so much of what follows is about pinning
conventions with tests rather than about handling errors.
...
Everything is implemented and verified. Parity against the reference,
measured on real weights where the component fits in a CPU sandbox and
on reduced-width models where it does not:

| Component | Measure |
|---|---|
| Autoencoder decoder | 131.5 dB PSNR, 131.2 dB non-square |
| Text encoder | within 3e-08, swept across padding levels |
| Diffusion transformer | within 3e-07, 12 configurations |
| Sampler | matches an analytic solution, not just another implementation |

What remains is not a phase but a dependency: **performance is measured,
not optimised further.** The execution layer's options are all proven
output-preserving, and the largest wins so far came from measurement
rather than from cleverness. In order of what actually moved:

- wrapping model entry points in `jax.jit` (35 compiled programs to 17,
  decode steady state 1.65s to 0.53s)
- splitting rather than replicating the text encoder (5.80 GiB per chip
  to 0.63, and it was an allocation failure before that)
- the persistent compilation cache (warm-up 137s to 9.3s)

Deliberately not done, and each for a stated reason:

- **Splash attention.** Waiting for a measurement showing attention is
  the bottleneck. It has never appeared as one.
- **Quantization.** The goal is output identical to the reference, and
  memory has not been the binding constraint since placement was fixed.
- **`NumericPrecision.HIGH` for the decoder.** Measured on TPU and it
  was *slower* to compile with identical execution time. The theory that
  fewer bfloat16 passes would shrink the graph did not survive contact
  with the hardware. Leave it at `HIGHEST`.
...
## 4. Hazards: attention, rotary and masking
...
## 5. Hazards: placement, memory and compilation
...
## 6. Hazards: checkpoints and tokenization
...
### Why there is no logging inside numeric functions
